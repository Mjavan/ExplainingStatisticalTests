...
class NT_Xent(nn.Module):
    ...
    def forward(self, z_i, z_j):
        ...
        sim = self.similarity_f(p1.unsqueeze(
            1), p1.unsqueeze(0)) / self.temperature

        sim_i_j = torch.diag(sim, self.batch_size)
        sim_j_i = torch.diag(sim, -self.batch_size)

        positive_samples = torch.cat(
            (sim_i_j, sim_j_i), dim=0).reshape(self.batch_size * 2, 1)
        negative_samples = sim[self.mask].reshape(self.batch_size * 2, -1)

        labels = torch.zeros(
            self.batch_size * 2).to(positive_samples.device).long()
...
class SimCLRTraining:
    ...
    def _prepare_directories(self):
        ...
        self.save_dir_mcmc = self.save_dir_ckpts / 'mcmc_samples' / \
            f'{self.args.optimizer}_{self.args.exp}'
    ...
    def _initialize_model_and_optimizer(self):
        backbone = resnet50(
            weights=ResNet50_Weights.IMAGENET1K_V2, progress=True)
        self.online_network = SimCLR(
            backbone, hid_dim=HID_DIM, out_dim=PROJECTION_DIM).to(self.device)

        if self.args.optimizer == 'adam':
            self.optimizer = optim.Adam(
                self.online_network.parameters(), self.args.lr, weight_decay=self.args.wd)
            ...
            for phase in ['train', 'val']:
                ...
                for (img1, img2) in dataloader:
                    ...
                    loss = self._train_step(
                        proj1, proj2, phase, batch_idx, cycle_batch_length, epoch)
                    del proj1, proj2
                ...
                if phase == 'train':
                    print(
                        f"Epoch {epoch}, Train Loss: {self.history[phase][-1]:.4f}")
                elif phase == 'val':
                    print(
                        f"Epoch {epoch}, Val Loss: {self.history[phase][-1]:.4f}")
    ...
    def _train_step(self, proj1, proj2, phase, batch_idx, cycle_batch_length, epoch):
        ...
        if phase == 'train':
            ...
            CycCosineSch(self.args.lr, batch_idx, cycle_batch_length,
                         self.args.n_sam_cycle, self.optimizer)
    ...
    def _post_epoch_operations(self, epoch):
        ...
        if self.args.save_sample and epoch >= self.args.epoch_st and \
            ...
            torch.save(self.online_network.state_dict(), os.path.join(
                self.save_dir_mcmc, f'model_{self.mt}.pt'))
            self.mt += 1
            print(
                f'Sample {self.mt}/{self.args.N_samples} taken at epoch {epoch}')
        ...
        plotCurves(self.history, os.path.join(self.save_dir_param,
                   f'exp_{args.exp}_{args.model_type}loss.png'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_type', type=str, default='cb',
                        choices=['', 'cb'], help='Type of model')
    parser.add_argument('--optimizer', type=str, default='sghm',
                        choices=['adam', 'sgd', 'sghm'], help='Optimizer to use')
    parser.add_argument('--aug', type=str, default='simclr',
                        choices=['simclr', 'contig'], help='Data augmentation type')
    parser.add_argument('--val_size', type=float,
                        default=0.2, help='Validation size')
    parser.add_argument('--lr', type=float, default=1e-1, help='Learning rate')
    parser.add_argument('--wd', type=float, default=1e-6, help='Weight decay')
    parser.add_argument('--epoch_noise', type=int,
                        default=40, help='Epoch to inject noise')
    parser.add_argument('--save_sample', type=bool,
                        default=True, help='Save samples or not')
    parser.add_argument('--epoch_st', type=int, default=0,
                        help='Epoch to start saving checkpoints')
    parser.add_argument('--cycle_length', type=int,
                        default=50, help='Cycle length')
