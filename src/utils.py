...
def save_cam_with_alpha(image, gcam, alpha=0.5):
    ...
    gcam_colored = plt.cm.jet(gcam_resized)[0, :, :, :3] * 255  # Apply colormap and select RGB channels
...
def overlay_heatmap_single(image, heatmap, img_idx, overlay_dir=None, alpha=0.5):
    ...
    rescale_image, overlaid_img = save_cam_with_alpha(image, heatmap, alpha=alpha)

    # Create a 1x3 grid of subplots with consistent aspect ratios
    fig, axs = plt.subplots(1, 3, figsize=(12, 4), gridspec_kw={"width_ratios": [1, 1, 1.2]})
    ...
    # Heatmap
    axs[1].imshow(heatmap[0], cmap="jet", aspect="auto")
    axs[1].axis("off")
    ...
    cbar = fig.colorbar(plt.cm.ScalarMappable(cmap="jet"), ax=axs[2], fraction=0.046, pad=0.04)
