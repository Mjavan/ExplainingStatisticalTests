...
class HeatmapOverlay:
    ...
    def _read_config(self, file_path="src/config.json"):
    ...
    def _load_heatmaps(self):
        ...
        base_path = os.path.join(self.heatmap_dir, f"{self.seed}_{self.args.exp}_{self.model}_{self.target_layer}")
        try:
            self.h_X = np.load(os.path.join(base_path, f"healthy_{self.relu}_heatmap.npy"), mmap_mode="r")
            self.h_Y = np.load(os.path.join(base_path, f"unhealthy_{self.relu}_heatmap.npy"), mmap_mode="r")
    ...
    def _dataloader_to_numpy(self, dataloader) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for img_batch, label_batch in dataloader:
            images.append(img_batch.cpu().numpy())  # Convert torch tensors to numpy arrays
            labels.append(label_batch.cpu().numpy())
        images_np = np.concatenate(images)  # Shape: (num_samples, height, width, channels)
    ...
    def _load_images(self):
        ...
        file_name = os.path.join(self.img_dir, f"{self.seed}_{h_size}_{unh_size}")
        os.makedirs(file_name, exist_ok=True)

        file_healthy, label_healthy = self._get_file_paths(file_name, "healthy")
        file_unhealthy, label_unhealthy = self._get_file_paths(file_name, "unhealthy")
        ...
        if os.path.exists(file_healthy) and os.path.exists(file_unhealthy):
            ...
            self.healthy_np, healthy_labels_np = self._dataloader_to_numpy(healthy_loader)
            self.unhealthy_np, unhealthy_labels_np = self._dataloader_to_numpy(unhealthy_loader)
    ...
    def prepare_overlay_directory(self):
        ...
        base_path = os.path.join(self.overlay_dir, f"{self.seed}_{self.exp}_{self.model}_{self.target_layer}")
    ...
    def visualise_overlay(self):
        ...
        end_idx = min(start_idx + self.args.batch_size, len(image_group))  # Ensure we don't go out of bounds
        ...
        for idx in range(start_idx, end_idx):
            ...
            overlay_heatmap_single(image, heatmap, idx, save_dir, alpha=self.args.alpha)


parser = argparse.ArgumentParser(description="Overlaying HeatMaps")
parser.add_argument("--exp", type=int, default=7, help="ID of the current experiment!")
...
parser.add_argument(
    "--alpha", type=float, default=0.4, help="The factor used to blend the Grad-CAM heatmap with the original image"
)
