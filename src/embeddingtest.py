...
class Test:
        ...
        try:
            if self.model == 'resnet50-imgnet':
                ...
                self.encoder = torch.nn.Sequential(
                    *list(backbone.children())[:-1]).to(self.device)
    ...
    def load_save_embeddings(self):
        ...
        n, m = self.sample_size.get(
            'healthy_size', 100), self.sample_size.get('unhealthy_size', 100)
        file_path = os.path.join(
            self.embed_dir, f'{self.seed}_{self.pre_exp}_{self.model}_{n+m}')
        ...
        if os.path.exists(path_X) and os.path.exists(path_Y):
            ...
            print(
                "Saving extrected embeddings as numpy arrays if they were not save before")
